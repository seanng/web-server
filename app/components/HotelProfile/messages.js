/*
 * HotelProfile Messages
 *
 * This contains all the text for the HotelProfile component.
 */
import { defineMessages } from 'react-intl';

export default defineMessages({
  header: {
    id: 'app.components.HotelProfile.header',
    defaultMessage: 'This is the HotelProfile component !',
  },
  hourlyRate: {
    id: 'app.components.HotelProfile.hourlyRate',
    defaultMessage: 'Hourly Rate',
  },
  amenities: {
    id: 'app.components.HotelProfile.amenities',
    defaultMessage: 'Amenities',
  },
  location: {
    id: 'app.components.HotelProfile.location',
    defaultMessage: 'Location',
  },
});
